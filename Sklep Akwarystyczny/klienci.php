...
<?php
    ...
    $query = "SELECT * FROM `klienci_tab` WHERE 1";
    $result = mysqli_query($polaczenie, $query);
    while ($row = mysqli_fetch_array  ($result))
    {
        echo '<table>';
         echo '<tr><th colspan="2">Klient</th></tr>';
         echo '<tr><td>ID</td><td>' .$row['id_klienta']. '</td></tr>';
         echo '<tr><td>Imię</td><td>' .$row['imie']. '</td></tr>';
         echo '<tr><td>Nazwisko</td><td>' .$row['nazwisko']. '</td></tr>';
         echo '<tr><td>Email</td><td>' .$row['adres_email']. '</td></tr>';
         echo '<tr><td>Adres</td><td>' .$row['adres']. '</td></tr>';
         echo '<tr><td>Numer telefonu</td><td>' .$row['nr_telefonu']. '</td></tr>';
         echo '</table>';
    }
    ?>
    <div class="panel">
    <a href="dodawanie.php" class="back-anchor">Dodaj</a>
    <form action="usuwanie.php" method="post">
    <label style="margin-left:41%" for="del_id">ID Klienta:</label>
